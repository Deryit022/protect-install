...
echo "🚀 Memasang proteksi Admin Mount (hanya Admin ID 1)..."
...
# Pastikan direktori ada dan permission wajar
mkdir -p "$(dirname "$REMOTE_PATH")"
chmod 755 "$(dirname "$REMOTE_PATH")"

# Tulis file MountController.php dengan proteksi admin ID 1
cat > "$REMOTE_PATH" << 'EOF'
<?php
...
class MountController extends Controller
{
    ...
    private function checkAdminAccess()
    {
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak: hanya Admin utama (ID 1) yang boleh akses Mount! ©Protect By @PutzzOfficial');
        }
    }
...
echo "✅ Proteksi Admin Mount berhasil dipasang!"
echo "📂 Lokasi file: $REMOTE_PATH"
echo "🗂️ Backup file lama: $BACKUP_PATH (jika sebelumnya ada)"
echo "🔒 Hanya Admin utama (ID 1) yang bisa mengelola Mount."
